...
import { AddLMSPayment } from '../../API/LMSApi';
...
const LMSPaymentAdd = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, actions) => {
      ...
      try {
        ...
        const result = await AddLMSPayment(values);

        if (result.status === true) {
          actions.resetForm(); // Reset form after successful submission
          navigate(`/lms/payment/listing/${Id}`); // Navigate to payment listing page
        } else {
  ...
  const handleGoBack = () => {
    navigate(`/lms/payment/listing/${Id}`); // Navigate back to payment listing page
  };

  return (
    <div>
      <Breadcrumb pageName="Offline LMS Payment Add" />
